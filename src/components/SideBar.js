...
function SideBar({onChildClick, visible}){
    return(
        <div className='bg-gunMetal'>
            <button id="toggle-sidebar" onClick={onChildClick} data-drawer-target="logo-sidebar" data-drawer-toggle="logo-sidebar" aria-controls="logo-sidebar" type="button" className="absolute top-2 right-7 p-2 mt-6 ml-3 text-sm  rounded-lg focus:outline-none focus:ring-2">
                ...
                <div className="h-full px-3 py-4 flex flex-col overflow-y-auto bg-gunMetal border-r-2 border-tiffanyBlue">
                    <div className="flex items-center pl-2.5 mb-20">
                        <Link to='/' className='flex'>
                            ...
                            <span className="font-title self-center text-3xl whitespace-nowrap text-white">FootStats</span>
                        </Link>
                    </div>
